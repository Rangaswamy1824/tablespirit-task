...
const Cart = () => {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:5000/getcart/${obj.usercon._id}`).then((res)=>{
      setCart(res.data)
  ...
  let dec=(id,qty)=>{
    if(qty>1)
    {
axios.put("http://localhost:5000/dec",{"_id":id}).then(()=>{
  setF((f)=>!f)
  ...
  let del=(id)=>{
    axios.delete(`http://localhost:5000/delcart/${id}`).then(()=>{
      setF(!f)
    })
  }
  let inc=(id)=>{
    axios.put("http://localhost:5000/inc",{"_id":id}).then(()=>{
